...
from .box_ops import box_cxcywh_to_xyxy, box_iou, generalized_box_iou, bbox_select_iou, wasserstein_loss
...
class RTDETRCriterionv2(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, boxes_weight=None):
        ...
        loss_giou1 = bbox_select_iou(src_boxes, target_boxes, xywh=True, GIoU=True).squeeze(-1)
        loss_giou1 = loss_giou1 if boxes_weight is None else loss_giou1 * boxes_weight

        loss_wasserstein = wasserstein_loss(box_cxcywh_to_xyxy(src_boxes), box_cxcywh_to_xyxy(target_boxes))
        losses['loss_giou'] = (loss_giou1 + 4 * loss_wasserstein).sum() / num_boxes
